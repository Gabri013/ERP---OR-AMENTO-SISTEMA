...
export default function VendaDetailPage() {
  ...
  if (!venda)
    return (
      <Layout>
        <div className="p-6 text-muted-foreground">Venda não encontrada.</div>
      </Layout>
    );
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[venda.status] ?? "bg-muted"}`}
            >
          ...
          {(venda as any).ordensServico?.length > 0 ? (
            <Button
              ...
              variant="outline"
              onClick={() =>
                setLocation(`/os/${(venda as any).ordensServico[0].id}`)
              }
            >
              <ClipboardPlus className="h-4 w-4 mr-1" /> Ver OS
            </Button>
          ) : (
            <Button
              size="sm"
              onClick={() => gerarOsMut.mutate({ id: venda.id })}
              disabled={gerarOsMut.isPending}
            >
              <ClipboardPlus className="h-4 w-4 mr-1" />{" "}
              {gerarOsMut.isPending ? "Gerando..." : "Gerar OS"}
            </Button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            ...
            <CardContent className="space-y-1">
              ...
              {venda.cliente?.cnpjCpf && (
                <p className="text-sm text-muted-foreground">
                  CNPJ/CPF: {venda.cliente.cnpjCpf}
                </p>
              )}
              {venda.cliente?.telefone && (
                <p className="text-sm text-muted-foreground">
                  Tel: {venda.cliente.telefone}
                </p>
              )}
            </CardContent>
          </Card>
