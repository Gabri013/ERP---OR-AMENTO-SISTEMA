    ...
    <div className="overflow-x-auto pb-2">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-3 min-w-[300px]">
        {columns.map((column) => {
          ...
          return (
            <div key={column.key} className="rounded-[8px] border border-white/10 bg-white/5">
              <div className="flex items-center justify-between border-b border-white/10 bg-transparent px-3 py-2">
                <div>
                  <h3 className="text-xs font-bold uppercase text-white">{column.title}</h3>
                  <p className="text-[11px] text-white/70">{items.length} O.S.</p>
                </div>
                <span
                  className={cn(
              ...
              <div className="space-y-3 p-3">
                {items.length === 0 && (
                  <div className="rounded-[8px] border border-dashed border-white/10 bg-white/5 p-3 text-center text-xs text-white/60">
                    Sem O.S. neste setor
                  </div>
                )}
                {items.map((item) => (
                  <motion.div
                    ...
                    className="rounded-[8px] border border-white/10 bg-white/5 p-3 shadow-[0_1px_2px_rgba(255,255,255,0.04)]"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-xs font-bold text-[#003D7A]">{item.number}</p>
                        <p className="mt-1 line-clamp-2 text-xs font-semibold text-white">{item.product ?? "Produto sob encomenda"}</p>
                      </div>
                      <GripVertical className="h-4 w-4 text-white/60" />
                    </div>
                    <p className="mt-2 truncate text-[11px] text-white/70">{item.client}</p>
                    ...
                    <div className="mt-3 flex items-center justify-between border-t border-white/10 pt-2">
                      <span className="text-[11px] font-medium text-white/70">{item.currentSector}</span>
