...
function serializeVenda(r: any, cliente?: any) {
  return {
    id: r.id, numero: r.numero, orcamentoId: r.orcamentoId,
    clienteId: r.clienteId, usuarioId: r.usuarioId, dataVenda: r.dataVenda,
    valorTotal: Number(r.valorTotal), desconto: Number(r.desconto),
    formaPagamento: r.formaPagamento, numParcelas: r.numParcelas,
...
router.get("/vendas", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
  const params = ListVendasQueryParams.safeParse(req.query);
  ...
  let result = rows.map(r => ({
    id: r.id, numero: r.numero, orcamentoId: r.orcamentoId, clienteId: r.clienteId,
    usuarioId: r.usuarioId, dataVenda: r.dataVenda, valorTotal: Number(r.valorTotal),
    desconto: Number(r.desconto), formaPagamento: r.formaPagamento, numParcelas: r.numParcelas,
    status: r.status, observacoes: r.observacoes, observacoesVenda: r.observacoesVenda,
...
router.post("/vendas", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
  ...
  const valorTotal = itens.reduce((sum: number, i: any) => sum + (Number(i.valorUnitario) * Number(i.quantidade)), 0);
  ...
  res.status(201).json({
    id: venda.id, numero: venda.numero, orcamentoId: venda.orcamentoId,
    clienteId: venda.clienteId, usuarioId: venda.usuarioId, dataVenda: venda.dataVenda,
    valorTotal: Number(venda.valorTotal), desconto: Number(venda.desconto),
    formaPagamento: venda.formaPagamento, numParcelas: venda.numParcelas,
    status: venda.status, observacoes: venda.observacoes,
...
router.get("/vendas/:id", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
  ...
  const ordens = await db.ordemServico.findMany({ where: { vendaId: p.data.id } });
  ...
  res.json({
    ...
    cliente: venda.cliente ? {
      id: venda.cliente.id, razaoSocial: venda.cliente.razaoSocial, nomeFantasia: venda.cliente.nomeFantasia,
      cnpjCpf: venda.cliente.cnpjCpf, cidade: venda.cliente.cidade, estado: venda.cliente.estado,
      telefone: venda.cliente.telefone, email: venda.cliente.email, observacoes: venda.cliente.observacoes,
      createdAt: venda.cliente.createdAt instanceof Date ? venda.cliente.createdAt.toISOString() : venda.cliente.createdAt,
    } : undefined,
    itens: itens.map(i => ({
      id: i.id, produtoId: i.produtoId, descricaoManual: i.descricaoManual,
      quantidade: Number(i.quantidade), valorUnitario: Number(i.valorUnitario), valorTotal: Number(i.valorTotal),
    })),
    ordensServico: ordens.map(os => ({ id: os.id, numero: os.numero, status: os.status })),
  });
});

router.patch("/vendas/:id", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
  ...
  try {
    const row = await db.venda.update({ where: { id: p.data.id }, data });
    const cliente = await db.cliente.findUnique({ where: { id: row.clienteId } });
    res.json({
      id: row.id, numero: row.numero, orcamentoId: row.orcamentoId,
      clienteId: row.clienteId, usuarioId: row.usuarioId, dataVenda: row.dataVenda,
      valorTotal: Number(row.valorTotal), desconto: Number(row.desconto),
      formaPagamento: row.formaPagamento, numParcelas: row.numParcelas,
...
router.post("/vendas/:id/gerar-os", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
