...
export default function OSDetailPage() {
  ...
  const addObsMut = useMutation({
    mutationFn: (data: {
      ...
      addObservacaoOS(data.osId, {
        observacao: data.observacao,
        tipoSetor: data.tipoSetor,
      }),
    onSuccess: () => {
  ...
  if (!os)
    return (
      <Layout>
        <div className="p-6 text-muted-foreground">OS não encontrada.</div>
      </Layout>
    );
  ...
  return (
    <Layout>
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full ${prioridadeBg[os.prioridade]}`}
              />
              <div>
            ...
            <span
              className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusColors[os.status] ?? "bg-muted"}`}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            {next && (
              <Button
                onClick={() =>
                  avancarMut.mutate({ id: os.id, data: { novaEtapa: next } })
                }
              ...
              {ETAPAS.map((e, i) => {
                ...
                return (
                  <div key={e} className="flex-1 min-w-[70px] text-center">
                    ...
                    />
                    <p
                      className={`text-xs font-medium ${active ? "text-primary" : done ? "text-green-600" : "text-muted-foreground"}`}
                    >
                      {etapaLabels[e]}
                    </p>
                  </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span>{formatDate(os.dataInicio)}</span>
              </div>
              {os.dataTermino && (
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Termino</span>
                  <span>{formatDate(os.dataTermino)}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
        ...
        {/* Itens / Produtos */}
        {(os as any).itens?.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Itens / Produtos</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {(os as any).itens?.map((item: any, i: number) => (
                  <div
                    key={i}
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Button
                ...
                disabled={addObsMut.isPending || !obsText.trim()}
              >
                Enviar Observação
              </Button>
            </div>
