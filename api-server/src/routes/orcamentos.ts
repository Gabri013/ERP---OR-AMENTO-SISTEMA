...
function serializeOrc(r: any, cliente?: any) {
  return {
    id: r.id, numero: r.numero, clienteId: r.clienteId, usuarioId: r.usuarioId,
    dataOrcamento: r.dataOrcamento, validade: r.validade,
...
router.get("/orcamentos", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
  ...
  let result = rows.map(r => serializeOrc(r, r.cliente));
...
router.post("/orcamentos", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
...
router.get("/orcamentos/:id", requireAuth, requireRoles(SALES_ROLES), async (req, res): Promise<void> => {
  ...
  const serializedItens = itens.map(i => ({
    ...
    produto: i.produto ? {
      id: i.produto.id, codigo: i.produto.codigo, nome: i.produto.nome,
      descricao: i.produto.descricao, foto: i.produto.foto, valor: Number(i.produto.valor),
      estoque: i.produto.estoque, status: i.produto.status,
    } : undefined,
  }));
